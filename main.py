...
# --- استيراد الملفات (تأكدي أن الأسماء في GitHub تطابق هذه الأسماء) ---
import responses, games, bank, ai_logic, whisper, protection, fun_ranks, identity, top_members, avatar, individual_info, bot_commands
import powers  # هذا هو ملف الرتب الجديد
...
def main_router(message):
    ...
    # توزيع المهام على الملفات
    protection.handle_protection(bot, message)
    powers.handle_ranks(bot, message) # لاحظي هنا استخدمنا powers
    ...
    if message.text in ["ايدي", "ا", "id"]:
        user_rank = powers.get_user_rank(message.chat.id, message.from_user.id)
